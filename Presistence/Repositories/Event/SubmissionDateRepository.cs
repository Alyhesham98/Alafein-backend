    ...
    internal sealed class SubmissionDateRepository : GenericRepository<SubmissionDate>, ISubmissionDateRepository
    {
        ...
        public async Task<(int Count, IList<ListEventDto>? Data)> GetEvents(EventListParameters parameters, Expression<Func<SubmissionDate, bool>>? filter)
        {
            var events = _context.SubmissionDates
                                 .Where(f => !f.IsDeleted).OrderByDescending(o => o.CreatedAt.Date);

            if (filter is not null)
            {
                events = events.Where(filter).OrderByDescending(o => o.CreatedAt.Date);
            }
            ...
            if (parameters.Name is not null)
            {
                ...
                events = events.Where(f => f.Submission
                                            .EventNameAR
                                            .Contains(search)).OrderByDescending(o => o.CreatedAt.Date);
                events = events.Where(f => f.Submission
                            .EventNameEN
                            .Contains(search)).OrderByDescending(o => o.CreatedAt.Date);
            }
            ...
            {
                events = events.Where(f => f.Submission
                                            .CategoryId == parameters.CategoryId).OrderByDescending(o => o.CreatedAt.Date);
            }

            if (parameters.IsApproved is not null)
            {
                events = events.Where(f => f.Submission
                                            .IsApproved == parameters.IsApproved).OrderByDescending(o => o.CreatedAt.Date);
            }

            if (parameters.IsSpotlight is not null)
            {
                events = events.Where(f => f.Submission
                                            .IsSpotlight == parameters.IsSpotlight).OrderByDescending(o => o.CreatedAt.Date);
            }

            if (parameters.IsPending is not null)
            {
                events = events.Where(f => f.Submission
                                            .Status == SubmissionStatus.PENDING).OrderByDescending(o => o.CreatedAt.Date);
            }

            var count = await events.CountAsync();

            var data = await events.Skip((parameters.PageNumber - 1) * parameters.PageSize)
                                   .Take(parameters.PageSize).OrderByDescending(o => o.CreatedAt.Date)
